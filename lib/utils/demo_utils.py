...
def download_segmentation(outdir='data/segm_data'):
    os.makedirs(outdir, exist_ok=True)

    weight_file = [
        'data/segm_data/decoder_epoch_25.pth',
        'data/segm_data/encoder_epoch_25.pth']
    urls = [
        'http://sceneparsing.csail.mit.edu/model/pytorch/ade20k-resnet101dilated-ppm_deepsup/decoder_epoch_25.pth',
        'http://sceneparsing.csail.mit.edu/model/pytorch/ade20k-resnet101dilated-ppm_deepsup/encoder_epoch_25.pth']
    for wf, url in zip(weight_file, urls):
        if not os.path.isfile(wf):
            cmd = ['wget', '-c', url, '-P', outdir]
            subprocess.call(cmd)


def download_crosswalk(outdir='data/segm_data'):
    weight_file = "data/segm_data/model.tflite"
    url = "https://drive.google.com/u/0/uc?id=1ZxObpKaG8bLLwvwBY0cmXj0ptHm-z54C&export=download"
